...
Video execution and visual workflow engines are intentionally deferred. The repo now supports optional local Diffusers hero-still generation, deterministic product-locked still compositing, and fallback-safe placeholder execution without installing ComfyUI, downloading bundled weights, or calling paid APIs.
...
`product_locked_composite.png` is now rendered as a manual-anchor alpha overlay. The hero still can be real or placeholder, but the product layer is loaded from product metadata and pasted as frozen pixels. The product is not generated by diffusion, relit, segmented, or modified beyond deterministic resize/rotation and alpha compositing.

Transparent product assets are generated with Pillow by:

```bash
python scripts/generate_product_alpha_assets.py
```

This creates deterministic non-branded `product_alpha.png` files for each handbag. These are tracked demo assets, not generated catalog outputs.

Placement is data-driven through action metadata. `standing_right_hand_visible` includes multiple anchor presets and a default anchor:
...
Override the anchor for a demo run:
...
V1 is intentionally simple: no shadows, no edge blending, no pose estimation, no relighting, and no background removal. Visual quality is tuned through contact sheets rather than automated pose logic.
...
Create an anchor tuning contact sheet:
...
The trace records product alpha status, alpha bounding box, anchor ID, default/override usage, x/y/scale ratios, rendered size, and `destructive_diffusion_allowed: false`.

## Why Not Video Yet?

The static product composite needs to be visually usable before video. Once still placement is credible, later passes can evaluate manual anchor tuning, simple shadows/edge blending, product projection, or image-to-video experiments without changing the product-fidelity rule.
...
Future passes can add shadow/edge blend v1, better real product cutouts, product freeze masks, ComfyUI workflow import, LTX image-to-video, hosted fallback routing, product-locked projection, richer evaluation, and a curated README demo.
